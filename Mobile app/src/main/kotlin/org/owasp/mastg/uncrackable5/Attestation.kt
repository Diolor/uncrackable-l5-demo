...
/**
 * Generates a throwaway EC P-256 signing key in AndroidKeyStore attested against the server
 * challenge, exports its certificate chain, signs the challenge with it, and deletes the key.
 *
 * StrongBox is tried first; on [StrongBoxUnavailableException] or a bare [ProviderException]
 * (several OEMs throw the latter, some only at `generateKeyPair()`) the partial alias is deleted
 * and generation is retried in the TEE. StrongBox absence is not a failure — the server accepts
 * `TrustedEnvironment`. A `Software` security level is rejected by the server, not here.
 */
class Attestation(context: Context) {
    ...
    fun prove(challenge: ByteArray): AttestationProof {
        ...
        try {
            ...
            // The attestation key lives for one attempt only.
